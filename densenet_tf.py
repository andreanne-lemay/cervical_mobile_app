...
def dense_block(x, blocks, name, dropout_rate):
  """A dense block.
  Args:
    x: input tensor.
  ...
  for i in range(blocks):
    x = conv_block(x, 32, name=name + '_block' + str(i + 1), dropout_rate=dropout_rate)
  return x
...
def conv_block(x, growth_rate, name, dropout_rate=0.0):
  ...
  x1 = layers.Conv2D(
      growth_rate, 3, padding='same', use_bias=False, name=name + '_2_conv')(
          x1)
  if dropout_rate > 0:
    x1 = layers.SpatialDropout2D(dropout_rate)(x1, training=True)
...
def DenseNet(
  ...
  x = dense_block(x, blocks[0], name='conv2', dropout_rate=dropout_rate)
  x = transition_block(x, 0.5, name='pool2')
  x = dense_block(x, blocks[1], name='conv3', dropout_rate=dropout_rate)
  x = transition_block(x, 0.5, name='pool3')
  x = dense_block(x, blocks[2], name='conv4', dropout_rate=dropout_rate)
  x = transition_block(x, 0.5, name='pool4')
  x = dense_block(x, blocks[3], name='conv5', dropout_rate=dropout_rate)
...
def DenseNet121(include_top=True,
  ...
  return DenseNet([6, 12, 24, 16], include_top, weights, input_tensor,
                  input_shape, pooling, dropout_rate, classes)
