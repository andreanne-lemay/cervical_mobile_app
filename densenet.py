...
class _Transition(nn.Sequential):
    def __init__(self, spatial_dims: int, in_channels: int, out_channels: int) -> None:
        ...
        # self.add_module("dummy_conv", conv_type(in_channels, in_channels, kernel_size=1, bias=False))
    ...
    def __init__(
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i == len(block_config) - 1:
                # self.features.add_module("dummy_conv", conv_type(in_channels, in_channels, kernel_size=1, bias=False))
        ...
        for name, m in self.named_modules():
            if name.endswith('dummy_conv'):
                nn.init.constant_(torch.as_tensor(m.weight), float(1. / m.in_channels))
